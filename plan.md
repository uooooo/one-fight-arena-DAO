...
## Phase 1 – Move Foundations (DeepBook Integration)
...
- [ ] Scaffold Move workspace under `move/open_corner` with package manifest. (1 hour)
- [ ] Implement MVP-only modules (`fighters`, `support`, `markets`, `yes_coin`, `no_coin`) covering: create fighter, open vault, deposit, mint supporter tier, create custom coins (YES/NO), create DeepBook pool, resolve market, redeem winning coins. (6-9 hours)
  - `fighters.move`: FighterProfile creation
  - `support.move`: SupportVault creation, deposit, SupporterNFT mint
  - `yes_coin.move` and `no_coin.move`: Custom coin creation per market
  - `markets.move`: Market creation, DeepBook pool setup, resolution, redemption (1:1 SUI exchange)
- [ ] Add Move unit tests for critical paths (create_vault, deposit, create_coins, resolve, redeem); defer perk automation/complex vault logic to stretch issues. (2-3 hours)
...
## Phase 2 – Frontend MVP (Rich UI with DeepBook Integration)
...
- [ ] Implement layout, navigation, hero, and persistent tab bar shared across routes. (2-3 hours)
- [ ] Integrate DeepBook SDK (`@mysten/deepbook-v3`): install package, set up DeepBookClient, create balance manager. (2-3 hours)
- [ ] Build `Markets` tab (Promotion Layer): market list, storyline groupings, order book UI (bid/ask display), place order modal, positions drawer (can read from mock/localnet data while Move stabilizes). (5-6 hours)
- [ ] Build `Support` tab (Sponsorship Layer): fighter carousel, SupportVault meter, Supporter NFT mint/deposit components (UI finished even if transactions mocked). (3-4 hours)
- [ ] Integrate Sui wallet-kit connect; abstract auth layer to swap in zkLogin later without UI changes. (1-2 hours)
- [ ] Implement transaction builders for DeepBook orders and redemption; can operate against mocked/localnet RPC endpoints. (2-3 hours)
